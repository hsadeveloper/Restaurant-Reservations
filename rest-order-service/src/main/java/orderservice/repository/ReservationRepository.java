...
public interface ReservationRepository extends JpaRepository<RestaurantTableEntity, Long> {
    ...
    List<RestaurantTableEntity> findAllByStatusAndCreatedAtBefore(ReservationStatus status, LocalDateTime time);
    ...
    List<RestaurantTableEntity> findByStatusAndCreatedAtBefore(ReservationStatus status, LocalDateTime time);
    
	boolean existsByCustomerIdAndReservationDateAndReservationTime(String customerId, LocalDate date, LocalTime time);
